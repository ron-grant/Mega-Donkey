/*  MegaDonkey Library File:  LCDSED.c    Epson S1D1335C LCD DRIVER     
 ...
 This .c file does not have an associated header file.
   ...
   Graphic LCD Interface Code (DRIVER) for Epson S1D1335C based panels
   Mark Sims  
   Jun 2007

 
  Note that the 1335 chip defines the screen such that DB0 is the leftmost
  pixel and DB7 is the rightmost.  For best efficency we connect the data bus
  up reversed and use the reverse table to swap command bits around.  Data
  accesses do not go through the reverse table.
...
//#define REVERSE_DB    // if you just have to have a normal data bus connection
...
//#define NOP asm volatile ("nop");  asm volatile ("nop"); 
// #define NOP asm volatile ("nop"); asm volatile ("nop");  asm volatile ("nop"); 
#define NOP asm volatile ("nop");asm volatile ("nop"); asm volatile ("nop");  asm volatile ("nop"); 

#define rw_low()     DATA_PORT_DDR=0xFF; cbi(PORTC,4);              NOP;
#define rw_high()    PORTA=0xFF; DATA_PORT_DDR=0x00; sbi(PORTC,4);  NOP;
#define rs_low()     cbi(PORTC,3);                                  NOP;
#define rs_high()    sbi(PORTC,3);                                  NOP;
#define cs_low()     cbi(PORTC,5); NOP; rs_high();                       
#define cs_high()    sbi(PORTC,5);                                  NOP;
#define e_low()      cbi(PORTG,2); NOP;NOP;  NOP;NOP;  /* e_low needs to be stretched */
#define e_high()     sbi(PORTG,2); NOP;NOP;  NOP;NOP;  /* e_high needs to be stretched */
...
#define sed_write_param(data) sed_write_data(reverse(data))

#define CUR_OFF() sed_write_cmd(SED1335_CMD_DISP_ON);   sed_write_param(0x04);
...
//u08 bzy;
//#define busy_wait() for(bzy=0; bzy<100; bzy++) if(PINC & 0x04) break;

// Making sed_write_cmd() and sed_write_data() macros for inline code 
// generation increases code size by over 6K.  Increases speed of 
// side_blit test by around 5 percent...

#ifdef INLINE_HW_ACCESS

#define sed_write_cmd(c)  \
{                         \
   rw_low();              \
   data_out(reverse(c));  \
   e_high();              \
...
#define set_addr(col, row)   \
{                            \
unsigned addr;               \
   addr = top_line + ((unsigned)(row) * (COLS>>3)) + (((unsigned)(col)) >> 3);  \
   sed_write_cmd(SED1335_CMD_CSRW); \
   sed_write_param(addr & 0xFF);    \
   sed_write_param(addr >> 8);      \
}

#else

void sed_write_cmd(u08 c)
{
   rw_low();

   data_out(reverse(c));
   e_high();      // clock the cmd byte
   e_low();
   rw_high();
}

void sed_write_data(char c)
{
   rs_low();      // select data register
   rw_low();
...
unsigned addr;
   ...
   addr = top_line + ((unsigned)row * (COLS>>3)) + (((unsigned)col) >> 3);    /* !!! * multiply */
...
unsigned addr;
   ...
   sed_write_cmd(SED1335_CMD_MWRITE);
   sed_write_data(c);

   CUR_OFF();  // mem writes turn hardware cursor on
   cs_high();
}
...
// set_addr(col, row);  // doing it inline:
   addr = top_line + ((unsigned)row * (COLS>>3)) + (((unsigned)col) >> 3);    /* !!! * multiply */
   ...
   sed_write_cmd(SED1335_CMD_MREAD);
...
#define SS ((COLS/PIXELS_PER_BYTE)*ROWS)
...
cur = erase_cursor();
   ...
   cs_low();
   sed_write_cmd(SED1335_CMD_SCROLL);
   sed_write_param(row&0xFF);      // screen block 1
   sed_write_param(row>>8);
   sed_write_param(ROWS);
   if(1) {
      if(row) row = 0;
      else row = SS;
//    sed_write_param(row & 0xFF); // screen block 2
//    sed_write_param(row >> 8);
      sed_write_param(0x00);       // screen block 2
      sed_write_param(0x60);
      sed_write_param(ROWS);
      sed_write_param(0x00);      // screen block 3
      sed_write_param(0x00);
      sed_write_param(0x00);      // screen block 4
      sed_write_param(0x00);
   }
   cs_high();
if(cur) show_cursor();
...
void lcd_xinit(u08 mode)
{
   ...
   DDRC |= 0xF8;  
   DDRG |= 0x04; 
   cbi(DDRC, 6);  // PORTC.6 is busy signal - not valid in MC6800 mode

   cs_high();           // deselect LCD
   rw_high();           // disable read mode
   rs_high();
   ...
   cs_low();
   sed_write_cmd(SED1335_CMD_SYSTEM_SET);
   sed_write_param(0x30);
   sed_write_param(0x87);
   sed_write_param(0x07);
   sed_write_param(0x27);
   sed_write_param(0x42);
   sed_write_param(0xEF);
   sed_write_param(0x28);
   sed_write_param(0x00);

   sed_write_cmd(SED1335_CMD_SCROLL);
   sed_write_param(0x00);      // screen block 1
   sed_write_param(0x00);
   sed_write_param(ROWS);
//   sed_write_param(SS & 0xFF); // screen block 2
//   sed_write_param(SS >> 8);
   sed_write_param(0x00);     // screen block 2
   sed_write_param(0x60);
   sed_write_param(ROWS);
   sed_write_param(0x00);      // screen block 3
   ...
   sed_write_cmd(SED1335_CMD_CGRAM_ADR);  // Set start address of char generator RAM
   sed_write_param(0x00);
   sed_write_param(0x70);

   sed_write_cmd(SED1335_CMD_HDOT_SCR);
   sed_write_param(0x00);

   sed_write_cmd(SED1335_CMD_OVLAY);
   sed_write_param(0x0C); // was 00

   sed_write_cmd(SED1335_CMD_DISP_OFF);
   sed_write_param(0x04); // no cursor,  only screen block 1 active

   sed_write_cmd(SED1335_CMD_CSRW);
   sed_write_param(0x00);
   sed_write_param(0x00);

   sed_write_cmd(SED1335_CMD_CSRFORM);
   sed_write_param(0x04);
   sed_write_param(0x86);

   sed_write_cmd(SED1335_CMD_DISP_ON);
   sed_write_param(0x04);
...
cur = erase_cursor();
   ...
   while(row_count--) {  /* for each row of the blit */
      ...
      if(draw_flags & BLIT_WRITE) {  // blit source is char buffer
         ...
         cs_low();
         set_addr(left, src_row);

         sed_write_cmd(SED1335_CMD_MREAD); 
         rw_high();
         for(temp=left; temp<=right+7; temp+=8) {  // read screen using auto reads
            e_high();       /* clock in the data */
      ...
      if(out_left < out_right) { /* use byte writes to zap out the middle of the line */
         if((draw_flags & (BLIT_AND | BLIT_OR | BLIT_XOR)) == 0) {  /* plain bit copy goes real fast */
            if(draw_flags & BLIT_READ) goto next_row;
            cs_low();
            set_addr(out_left, dest_row);
