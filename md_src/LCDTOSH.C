/*  MegaDonkey Library File:  LCDTOSH.c    LCD DRIVER for Toshiba T6963C based panels     
   ...
   Graphic LCD Interface Code for Toshiba T6963C based panels
   Mark Sims  Jun 2007
 ...
 Note that the T6963 defines the screen such that DB0 is the leftmost
...
#define wr_low()     cbi(PORTC,4); NOP; NOP;
#define wr_high()    sbi(PORTC,4); PORTA=0xFF; DATA_PORT_DDR=0x00;
#define rd_low()     cbi(PORTG,2); NOP; NOP;
#define rd_high()    sbi(PORTG,2); 
...
#define SET_TXHOME  0x40  // set text mode home address (top displayed line)
#define SET_TXAREA  0x41  // set text mode screen area
#define SET_GRHOME  0x42  // set graphics home address (top displayed line)
#define SET_GRAREA  0x43  // set graphics screen area
#define SET_OFFSET  0x22  // set data read/write offset pointer
#define SET_ADP     0x24  // set data read/write address pointer
#define OR_MODE     0x80  // OR mode for mixed text/graphics
#define GR_MODE     0x98  // graphics mode
#define TX_MODE     0x97  // text mode, blinking cursor
#define CURSOR_SIZE 0xA0  // text cursor size
#define AUTO_WRITE  0xB0  // enable auto screen writes
#define AUTO_READ   0xB1  // enable auto screen reads
#define AUTO_OFF    0xB2  // disable auto screen access mode
#define DATA_WRITE  0xC0  // write data to screen, increment addr
#define DATA_READ   0xC1  // read data from screen, increment addr
#define CLR_BIT     0xF0  // clear data bit
#define SET_BIT     0xF8  // set data bit

/* ----------------- LCD controller command routines ----------------- */

#ifdef INLINE_HW_ACCESS

#define status_check(mask)                    \
{                                             \
u08 i;                                        \
u08 c;                                        \
   rs_high();                                 \
   for(i=0; i<100; i++) {                     \
      rd_low();                               \
      c = reverse(DATA_PORT_READ);            \
      rd_high();                              \
      if((c & (mask)) == (mask)) break;       \
   }                                          \
}
...
{                            \
   status_check(0x03);       \
   data_out(reverse(cmd));   \
   wr_low();                 \
   wr_high();                \
}


#define auto_off(status)         \
{                                \
   status_check(status);         \
   data_out(reverse(AUTO_OFF));  \
   wr_low();                     \
   wr_high();                    \
}


#define send_word(word)               \
{                                     \
   status_check(0x03);                \
   rs_low();                          \
   data_out(reverse((word) & 0xFF));  \
   wr_low();                          \
   wr_high();                         \
   status_check(0x03);                \
   rs_low();                          \
   data_out(reverse((word) >> 8));    \
   wr_low();                          \
   wr_high();                         \
}
...
{                         \
unsigned addr;            \
   addr = top_line + (row * (COLS>>3)) + (col >> 3);\
   send_word(addr);       \
   send_command(SET_ADP); \
}

#else

void status_check(u08 mask)
{
u08 i;
u08 c;

   rs_high();    // access status reg
   for(i=0; i<100; i++) {
      rd_low();
      c = reverse(DATA_PORT_READ);  //!!!!
      rd_high();
...
void send_command(u08 cmd)
{
   status_check(0x03);    // exits with RS high
...
unsigned addr;
   ...
   addr = top_line + (row * (COLS>>3)) + (col >> 3);    /* !!! * multiply */
   send_word(addr);
   send_command(SET_ADP);  // set address pointer
}
...
cur = erase_cursor();
   ...
   if(text_mode) {send_command(SET_TXHOME);}
   else {send_command(SET_GRHOME);}
...
void lcd_xinit(u08 mode)
{
   ...
   wr_high();           // disable read mode
   rd_high();           // disable write mode
   rs_high();
   ...
   send_word(0);              // set text mode home address
   send_command(SET_TXHOME);
   send_word(0);              // set graphics mode home address
   send_command(SET_GRHOME);

// send_command(AUTO_OFF);    // reset auto mode

   send_word(0 | (COLS>>3));
   send_command(SET_TXAREA);  // set text area

   send_word(0 | (COLS>>3));
   send_command(SET_GRAREA);  // set graphic area

   send_command(OR_MODE);     // OR mode if mixing text/graphics
   send_command(mode);        // graphics on, text off
...
cur = erase_cursor();
   ...
   cs_low();
   set_addr(0, 0);
   send_command(AUTO_WRITE);

   count = ROWS * (COLS>>3);
   while(count--) {   /* fast fill screen memory using auto writes */
      status_check(0x08);
      rs_low();
...
cur = erase_cursor();
   ...
   while(row_count--) {  /* for each row of the blit */
      ...
      if(draw_flags & BLIT_WRITE) {  // blit source is char buffer
         ...
         send_command(AUTO_READ);

         for(temp=left; temp<=right+1; temp+=8) {  // read screen using auto reads
            status_check(0x04);
            rs_low();
